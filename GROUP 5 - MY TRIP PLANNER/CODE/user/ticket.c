...
void main()
{
    ...
    details(j);
    printf("Enter The Source Place and Destination place: ");
    fflush(stdin);
    gets(source);
...
void details(int k)
{
    ...
    char val[20], gen[6];
    for (i = 1; i <= k; i++) {
        printf("Enter The %dth Passenger Name and Gender: ", i);
        fflush(stdin);
        gets(val);
...
void bill(int y, int j)
{
    ...
    Node* ptr = start;
    for (i = 1; i <= j; i++) {
        printf("\t\t\%dst Passenger Name Gender: ", i);
        puts(ptr->name);
    ...
    printf("\t\tSource Place and Destination Place : ");
    puts(source);
